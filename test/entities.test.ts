...
describe("entity index over gated text (C9)", () => {
  ...
  it("indexes attachment testimony once the drain gates it", async () => {
    ...
    await callApp(
      env,
      `/api/intake/${id}/attachments?filename=scan.png&media_type=image/png&access_code=${encodeURIComponent(code)}`,
      { method: "POST", headers: { "content-type": "image/png" }, body: "fake-scan" },
    );
  ...
  it("resolves links by HMAC against sealed entities, without names", async () => {
    ...
    const res = await callApp(env, `/api/intake/entities/links?hmac=${hmac}`, {
      headers: auth,
    });
